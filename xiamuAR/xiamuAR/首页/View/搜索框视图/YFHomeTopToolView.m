...
/* 通知 */
@property (weak ,nonatomic) id YFNotification;
@end
@implementation YFHomeTopToolView
...
- (void)makeNotification
{
    //滚动到详情
    YFWeakSelf(ws)
    _YFNotification = [[NSNotificationCenter defaultCenter]addObserverForName:SHOWTOPTOOLVIEW object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        ws.backgroundColor = [UIColor whiteColor];
        _topSearchView.backgroundColor = LZColorFromRGB(240, 240, 240);
        [ws.leftItemButton setImage:[UIImage imageNamed:@"shouye_icon_scan_gray"] forState:0];
        [ws.rightItemButton setImage:[UIImage imageNamed:@"shouye_icon_sort_gray"] forState:0];
        [ws.rightRItemButton setImage:[UIImage imageNamed:@"icon_gouwuche_title_gray"] forState:0];
    }];
    
    _YFNotification = [[NSNotificationCenter defaultCenter]addObserverForName:HIDETOPTOOLVIEW object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        ws.backgroundColor = [UIColor clearColor];
        _topSearchView.backgroundColor = [UIColor whiteColor];
        [ws.leftItemButton setImage:[UIImage imageNamed:@"shouye_icon_scan_white"] forState:0];
        [ws.rightItemButton setImage:[UIImage imageNamed:@"shouye_icon_sort_white"] forState:0];
        [ws.rightRItemButton setImage:[UIImage imageNamed:@"icon_gouwuche_title_white"] forState:0];
    }];
}
...
- (void)dealloc
{
    [[NSNotificationCenter defaultCenter]removeObserver:_YFNotification];
}
